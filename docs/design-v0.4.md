...
- **本文是目标架构**。已实现的 MVP 比它小得多——按第一性原理砍掉了什么、为什么、什么时候加回来，见 **§15.0**；第二波见 **§15.0.1**；第三波（pipeline 回到 action 列表、guard、数据集发布）见 **§15.0.2**；第四波（命令的信任边界与可移植性）见 **§15.0.3**；第五波（进度可见性，以及为什么它不是引入 Temporal 的理由）见 **§15.0.4**；第六波（复用、反复修复、一处正确性 bug）见 **§15.0.5**；第七波（最小提交要配多少）见 **§15.0.6**；第八波（官方 action 的边界）见 **§15.0.7**；第九波（report 是出口上的一环）见 **§15.0.8**；实现与用法见 `README.md`
...
| 项 | 约定 |
|---|---|
| 三种写法 | `run`（argv 数组，用 `{{.Key}}` 模板）、`script`（内联 sh）、`uses`（**一个可执行文件**，配 `sha256` 钉住）。变量以同名大写环境变量传入（`KEY` / `LOCAL_PATH`，git 来源另有 `GIT_REPO` / `GIT_REF`） |
| `uses` 是可执行文件，不是 shell 脚本 | 文件被**直接执行**，由它自己的 `#!` 决定用什么解释——Python、编译好的二进制都行。用 shell 去读它会强迫每个适配器都是 sh，并且**静默忽略它自己声明的解释器**。文件必须可执行且有 `#!`，两者都在运行前检查并给出一行修法 |
...
**自定义步骤不需要新语法**：pipeline 里 `uses:` 认不出来的名字就回落到这里的命令，因此通知、二次归档、投递到别的系统都是"配一条命令"，并且继承命令已有的哈希 pin 与 env 白名单（§15.0.2 / §15.0.3）。

**关于可移植性，三件事要分开说**（§15.0.3）：

1. **跨 Linux/macOS 真正会坏的很少是 shell，是那些工具**：`readlink -f`、`sha256sum`、`stat -c`、`date -d`、`base64 -w`、`grep -P`、不带参数的 `sed -i` —— 这些 GNU 拼法在 BSD userland 上要么报错要么悄悄做别的事。smoke test 直接 grep 这批模式，适配器不可能不知不觉带进来一个。
2. **适配器本来就是绑在某台机器的工具链上的**：`ship-rclone` 要一份配好的 `rclone.conf`，`harbor` 要 harbor 和 Docker daemon。脚本用什么语言，远不是它最紧的耦合。
3. **有些东西无论用什么语言都只能在 Linux 上**：`local` executor 和 `fake-harbor.sh` 用 `unshare --mount`，mount namespace 没有 macOS 等价物。**运行端就是面向 Linux 的**，把这句说清楚，比暗示"是脚本语言挡着"要诚实。
...
这一波的净效果：删掉手写的 docker runtime，Go 代码基本持平，依赖不变（yaml + toml），smoke test 从 28 条断言涨到 61 条。
...
**顺便查出 `test/cases/libaom-...` 这个 case 当时是坏的**：`solution/testcase.bin` 是一个 Git LFS 指针（`size 194`），而 LFS 对象在服务端 404 —— 指针提交了，二进制没推上去。oracle 拿到的是 128 字节的指针文本而不是 194 字节的 PoC，目标进程正常退出，得 0 分。**准入闸门做了它该做的事：拒绝这个 case，而不是把 0 分当成一次测量报出去。**
...
### 15.0.3 第四波：命令的信任边界与可移植性

两个问题，答案都不在"换个打包形式"上。

#### 「适配器做成插件是不是更安全」

**让 GitHub Actions 安全的不是打包形式**，是三件可以拆开做、且都很便宜的事。marketplace 那一层是**分发**，不是安全，一台机器的时候是纯负担。

| 属性 | 实现 |
|---|---|
| `@sha` —— 跑的是不是我审过的那份 | `uses:` 指向文件，`sha256:` 钉住。**对不上就拒绝执行，不是告警**——没人会去看的告警不是控制。每次 run 另写 `manifest.json`，记下跑了哪些命令、哈希多少、是否来自可信文件 |
| 最小权限 | `inherit_env: false` + 每条命令声明 `env:`。ship 命令跟 git 说话，就不该顺手拿到模型厂商的 key |
| **谁定义步骤 ≠ 谁调用步骤** | `--commands FILE`：命令只能来自运维方那个文件；提交里再带一份 `kind: Commands` **直接拒绝**——合并等于让提交方重定义 `ship`，静默丢弃等于让他以为自己那份跑了 |

第三条才是核心问题：**谁能给你一份 YAML，谁就能在你的 runner 上执行任意代码**，因为提交里可以自带命令。提交方选跑哪些步骤（`using: ship`），运维方决定步骤**是什么**。

**沙箱化适配器没做，那基本是表演**：适配器的活就是驱动 Docker/Harbor，它必须拿到 daemon socket，而 daemon socket 本来就等价于 root。

#### 「适配器全是 sh，可移植性是不是不行」

前提对了一半，而对的那一半修法比想象的小得多。**真正的 bug 不是"sh 不可移植"，是我们强制适配器必须是 shell 脚本**：

```go
cmd = exec.CommandContext(ctx, shell(), c.Uses)   // 它自己的 #! 被忽略了
```

契约是「环境变量进、文件出」，没有任何一条说 shell。改成直接执行文件、由 `#!` 决定解释器之后，适配器可以是 Python、编译好的二进制、任何东西 —— 这才是真正买到可移植性的那一下，而且是三行。附带修掉一个隐蔽的脆弱点：原来 `shell()` 是"有 bash 用 bash，没有就用 sh"，而我们自己的适配器用了 bash 数组，在没装 bash 的机器上会**静默换一个解释器然后坏掉**。

剩下的三件事见 §9.4 的可移植性小节：会坏的是工具不是 shell、适配器本来就绑在机器的工具链上、以及有些部分无论用什么语言都只能在 Linux 上。

smoke test 相应加了第 14 步：每个适配器可执行且有 `#!`、没有 GNU-only 拼法、shell 的能解析，外加一个**Python 写的 pipeline 步骤**证明这个格式确实不是 shell 形状的，以及一个不可执行的适配器会不会给出那行修法。

### 15.0.4 第五波：进度可见性 —— 以及为什么它不是引入 Temporal 的理由

**症状**：pipeline 步骤变多之后，日志只在 trial **结束**时说一句话。中间是静默的，没有分母，也看不出某个 case 走到哪了。而「汇报状态」本来就是四个动词之一。

**三个问题，三处答案**：

| 问题 | 答案 |
|---|---|
| 一共多少 | matrix 展开后立刻把总数和**每个 case 的分母**写进 `progress.json`；`case 2/5 …` 在解析时就报 |
| 完成了多少 | 每条完成日志带 `[7/34]`；`progress.json` 同时记 done / dropped / failed |
| 现在在干什么 | 心跳每 15s 打一行：`6/34 done · 4 running (harbor×2, archive×1, ship×1)`，加一行 per-case 进度；在飞的最久的那个超过 1 分钟就单独点名 —— 那是「是不是卡住了」的答案，也是唯一值得点名的一个 |

**为什么状态放在文件里而不是只打日志**：日志是个很差的查询目标 —— 写一次、靠翻页读、而且不说现在怎么样。`progress.json` 随状态变化重写（**用 rename 落盘**，读者永远不会看到写了一半的文件 —— 这恰恰重要，因为预期用法就是一个进程写、另一个进程读），`rollout-man status` 因此在**运行中**也能用，`--case` 可以只看一个 case。

#### 值不值得为了这个可视化引入 Temporal

**不值得，而且这个判断和 §15.0 的判据是同一个。**

Temporal Web UI 确实好：每个 workflow 的时间线、activity 级别的起止与重试、按 id 或 search attribute 查、崩溃后的历史回放。问题在于**那是 durable execution 的副产品**。要副产品就得付全款：

1. **多一个 server 要养**，而且按 §14.1，`start-dev` 不等价于 docker-compose + PG persistence，所以是「Temporal + PostgreSQL」两个进程。
2. **determinism 与 versioning 纪律**要一直守着。
3. **最要命的一条**：Temporal 的 UI 看到的是 **activity**。想在它上面看见「trial X 正在 archive 这一步」，就得把每个 action 拆成 activity —— 也就是**为了让可视化工具看得见，去重构 pipeline 的结构**。让工具决定架构，方向反了。

而上面那三个问题，是约 250 行的一个跟踪器加一个 JSON 文件解决的，没有新进程、没有新依赖。

**诚实的另一面**：有一件事 Temporal 能给而这套给不了 —— **跨 run、跨机器的检索**（「过去一个月所有 batch 里 case X 的全部 trial」）。但那是数据库的能力，不是编排器的；§15.0 早就写了判据：**多人共享结果时加回数据库**。所以那一天到来时，正确的动作是把已经是机器可读的 `results.jsonl` + `progress.json` 灌进一个库，而不是为了一个 UI 引进一个工作流引擎。

**§15.0 的判据不变：placement 落地时加回 Temporal** —— 那时才真的出现 lease / orphan / 选主，才轮到它替代最难写对的代码。

### 15.0.5 第六波：复用、反复修复，以及一处正确性 bug

从最近这批改动里查出的问题，按严重程度排。

#### 正确性：修复过的 case，记录的版本不是被测量的字节

`per_case` 的 `fix:` 步骤**原地改 case 目录**（适配器注释里写得很明白："a live source dir, not a copy"），而 `c.SHA256` 是在这之前算的。实测：

```
记录进 results.jsonl 的        b947c590f01b99d2
实际被跑的内容哈希             c6b7a5faeb7cfe55
```

**这直接推翻整份设计所依赖的那条不变量**（§3.1「内容哈希才是版本」）：dataset card 把这个哈希当作出处发布出去，而它指向的字节从来没有被任何 trial 跑过。

连带第二个后果：gate cache 也是用改之前的哈希做键，于是**下一次运行必然 miss**，重新审计、重新修复。实测三次运行，`marker.txt` 从 1 行长到 4 行 —— 修复不幂等时这就是累积损坏。

修法：`per_case` 跑完之后重新算哈希，变了就以新的为准并明说；缓存条目同时挂在修复前后两个哈希下（前者让"把目录改回去"不会重跑一次已经判过的修复）。

#### gate cache 的作用域和指纹都不对

- **作用域**：缓存写在 run 目录里，所以只对同一个 `--id` 的续跑有用，而每个新批次都换 id —— 它要避免的那个昂贵审计，实际上每次都还是跑了。裁决绑的是「内容哈希 + 检查指纹」，两者都跟 run 目录无关，缓存因此挪到 `runs/` 旁边。
- **指纹**：指纹没有包含**执行链**。准入探针是**通过 executor 跑的**，而「过了闸门也就证明整条链在这台机器上是通的」是闸门存在的理由之一 —— 用 fake harbor 得到的裁决不能拿给真 harbor 用。已把 executor 步骤及其解析到的命令一起算进指纹。
- 缓存只能对**写下来的东西**取键。环境变量、外部世界的状态它都看不见，所以补一个 `--regate` 显式重判。

#### 检查失败被当成瞬时故障重试

pipeline 步骤走的是 `cmdrun.Run`，带 `max_attempts` 重试。于是一个失败的检查跑 3 次、修复后再跑 3 次 —— **一个调模型的审计因此被计费 6 次**。

**一个说"不"的检查是裁决，不是事故** —— 这正是失败分类法对 agent 划的同一条线。pipeline 步骤现在只跑一次，确实是瞬时故障的步骤（上传）用 `retries:` 显式打开。

#### `fix` 只有一轮

`fix_attempts:`（默认 1）。多于一轮是给**收敛型**修复用的：让 agent 去补检查指出的问题，第一遍常常只补一半。

#### 复用：`include:`

五份提交各自重述同一批命令，其中一份把 `adapters/harbor.sh` **逐字复制**了 148 行进 yaml —— 适配器改一次要改两处，然后就漂移了。`kind: Commands` 加 `include:`，共享的适配器进 `experiments/commands.yaml`，提交只写不一样的那部分。

这不打开任何新口子：没有 `--commands` 时提交的命令本来就是可信的；有 `--commands` 时提交自带的 `Commands` 文档整份被拒，所以真正会被执行的 include 只有运维方自己的（§15.0.3）。

实测：experiments 从 1684 行降到 1346 行，十份提交全部能加载 —— 其中 `tcbatch1-smoke.yaml` 之前**根本解析不了**（写的是早已不存在的 pipeline 形状），也就是说它从来没被跑过。

#### 其余

- `upperSnake("CaseSHA")` 得到 `CASE_S_H_A` —— 缩写被拆开，仓库里已有的单测是红的。改成在词边界断（`CASE_SHA` / `RUN_ID` / `HTTP_SERVER`）。
- 并发化 `perCase` 时丢掉了 `case N/M` 计数器。闸门是一个批次里最慢的一段（每个 case 一次审计加一次修复），「一共多少、走到第几个」在**它进行当中**就得答得上来。
- gate cache 用 `os.WriteFile` 直写，崩在中间会留下截断的文件然后静默重判所有 case。改成 rename 落盘，和 `progress.json` 一致。

### 15.0.6 第七波：从产品角度问"要配多少东西"

把「对一个目录的 case 跑一遍」当成产品问题量一下，答案是三个缺口，都不大但都挡路。

| 缺口 | 之前 | 现在 |
|---|---|---|
| **没有 glob** | `cases:` 只能一条一条列。一个 24 个 case 的目录就是 24 行要维护的清单 | `path: "cases/*"` 展开；只认真的含 `task.toml` 的目录，免得 README 和残留压缩包变成 case |
| **路径只相对于 CWD** | 同一份提交只能从一个目录跑 | 先按 CWD 试，再按提交文件所在目录试。**注意这只对写成文件相对路径的提交有用**——仓库里现有那些写的是仓库根相对路径，仍然只能从仓库根跑 |
| **`pipeline.per_trial` 必须写** | 只有一个可能的执行器时也得重述一遍 | 缺省填上；写了 pipeline 的时候「第一步必须是执行器」这条不变 |

量出来的结果：**最小可跑的提交是 5 行**（本地执行器），**接 Harbor 是 8 行**（多一个 `kind: Commands` 声明适配器）。其余全部有默认值：`source: local`、`trials: 1`、`concurrency: 1`、`pipeline.concurrency`、`per_case_concurrency`。

准入闸门、清洗、guard、归档、发布——一个都不默认开。**这是有意的**：每一条都是一个应该被专门决定的事，默认打开等于替使用者做了判断，而这些判断错了都不便宜（准入错了数字不可信，清洗漏了不可逆）。

另外补一条：命令的键名现在也拒绝未知项。`sha256:` 去掉之后，如果它还留在某份提交里，本来会被**静默忽略**——一个看起来还在生效的设置，是比报错更糟的状态。

### 15.0.7 第八波：官方 action 的边界在哪

上一波用 `include:` 解决"五份提交重述同一批命令"，方向错了 —— 那些东西不是"该被共享的脚本"，是**本来就该内置的步骤**。`include:` 撤销，改成把够格的做成官方 action。

**判据：一件事是不是 action，看它属于谁。**

| 归属 | 结论 | 例子 |
|---|---|---|
| **case 格式 / 结果语义 / 通用传输** —— 属于这个工具 | 内置 | `check_case`、`report`、`ship_hf` / `ship_github` / `ship_rclone` |
| **某个项目的约定与策略** —— 属于提交方 | 留在 adapter | `forge_flow_fuzz`、`materialize_week2`、`check_traj_layout` |

按这条，这一波内置了三类：

- **`check_case`**：case 包是不是 Harbor 认得的形状。之前是一段 shell **用 grep 解析 TOML** —— 会命中被注释掉的键、看不见换了表的键、也分不清空值和缺失。而 rollout-man 本来就在读 task.toml，直接问它既短又对。特别是缺 `environment/` 这条：Harbor 会静默退化成 dataset 模式，扫目录、找不到任务，最后报一句和这个 case 毫无关系的 "Either datasets or tasks must be provided"。
- **`report`**：设计文档 §6.5 / §11.2 点名过、一直没实现。写出这一批发生了什么（md / json / csv）。`pass_at` 是**这份报告问的那个问题**，所以它被写进文件里——答案不该脱离问题被读到。
- **三个传输**：`ship_hf` / `ship_github` / `ship_rclone`。

**传输内置了，§9.4「不内置任何存储 SDK、不管理凭证」有没有被破坏？没有。** 它们驱动的还是 `hf` / `rclone` / `git` 这三个 CLI，一行 SDK 代码都没进来，凭证还是各自的工具自己去找。变的只是**谁带这段胶水**：工具带，而不是每份提交抄一遍脚本。而且它们各自声明自己这条传输需要哪些宿主变量（`HF_TOKEN`、`RCLONE_CONFIG`…），运维方不必为一个工具本来就知道需求的步骤去枚举。

**它们只做传输。** 送什么是上一步（`dataset` / `archive` / `path:`）决定的。把"哪些文件才对"塞进传输里，就是把一个项目的口味变成所有人的 —— 现有那几个 `ship_week2_*` 适配器正是这样长出来的。够不到的地方仍然是 `ship` + `using:`。

**两个模板**（`templates/`），区别就在最该讲清楚的那一点上：

- `action-tool.sh` —— 确定性本地工具。要能跑两遍，别把密钥写进 `$RUN_DIR` 下的任何东西。
- `action-llm.sh` —— 要调模型的。**两种失败从里面看长得一样，但绝不能同等对待**：模型答了、答案是"不"，那是**裁决**（步骤失败正是目的，重试是花钱买同一个答案）；调用没完成（超时、限流、没凭证），那是**事故**（这个才该配 `retries:`）。搞反了两个方向都贵：重试裁决把一个答案计费 N 次，把故障当裁决则否掉了从没被判过的工作。

两个模板**在被改写之前都必然失败**——一个悄悄成功的模板会被抄进 pipeline 然后什么也不做。

另外加了 `rollout-man actions`：列出内置步骤和各自的作用单位。**"这里能用什么"应该从工具问出来，而不是从文档**——文档才是会过期的那份拷贝，smoke test 现在两个方向都对一遍。

### 15.0.8 第九波：report 是流水线的最后一环，不是流水线里的一步

上一波把 `report` 做成了官方 action，但它当时和别的步骤一样：写在 `per_experiment` 里，轮到它才跑。问题是**轮不到它的那些批次，恰恰是最需要记录的批次**。

`ship` 挂了、gate 把所有 case 都判死了、有人 Ctrl-C —— 这三种批次的共同点是：**没人在旁边可以问「刚才那批到底怎么样」**。而这正是报告存在的理由。

所以 `report` 从「列表里的一项」改成「**出口上的一环**」：`Runner.Run` 用 `defer` 在每条退出路径上写它，包括 gate 前就失败的那条。

**三个不那么显然的决定：**

1. **收尾报告用的是提交自己声明的那份配置。** 如果提交写了 `- uses: report with: {dest: summary.md, pass_at: 0.8}`，收尾写的就是 `summary.md`、就是 `pass@0.80` —— 不是在旁边另写一个默认的 `report.md`。**问题和答案不该被拆开**：`pass_at` 是这份报告问的那个问题，一份没有问题的默认报告回答不了它。

2. **干净跑完的批次不会被写两遍。** 声明的那一步跑过了就已经把话说完了，收尾跳过。这一条是靠 action 自己把写过的路径报给 runner（`Ctx.Wrote`）实现的，而不是靠 runner 猜文件名 —— 实现时正是这里露出一个 bug：`append: true` 的分支提前 return，从来没报告过自己写了文件，于是收尾又追加了一份。smoke test 现在盯着这条（`^# ` 标题数）。

3. **但跑到一半才停的批次要被改写。** 声明的 report 先跑、之后 `ship` 挂了，那份文件说这批完成了，而它没有。这时收尾会**重写**，加上那一行：

   ```
   **This run did not complete.** ship: exit status 3: the bucket is gone
   ```

   `json` 用 `incomplete` 字段说同一件事；`csv` 保持是一张干净的表，不塞注释行 —— 它是给程序读的。

**Ctrl-C 是第二种「停在半路」，而且是不会自报家门的那种。** 被打断的 trial 会被如实记成 `HOST_ERROR`、进不了分母，这部分本来就是对的；但表头看上去和跑完的批次一模一样。所以收尾除了看返回的 error，还看 `ctx.Err()`，据此写「interrupted before every trial ran」。同理，收尾那次调用用 `context.WithoutCancel` —— **报告说的是那次运行，它必须能在承载那次运行的 context 已经没了之后才写出来**。

**测量和发布是两件事，这一波把它记进了行为里。** 停得晚只损失 ship，不损失数字：`ship` 失败的那份报告里，测量结果照样在。这和 §3.5 的 guard（`on_violation: drop` 决定发什么，从不决定观察到了什么）是同一条原则的另一面。
