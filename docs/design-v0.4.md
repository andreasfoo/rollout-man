...
- **本文是目标架构**。已实现的 MVP 比它小得多——按第一性原理砍掉了什么、为什么、什么时候加回来，见 **§15.0**；第二波见 **§15.0.1**；第三波（pipeline 回到 action 列表、guard、数据集发布）见 **§15.0.2**；第四波（命令的信任边界与可移植性）见 **§15.0.3**；第五波（进度可见性，以及为什么它不是引入 Temporal 的理由）见 **§15.0.4**；第六波（复用、反复修复、一处正确性 bug）见 **§15.0.5**；第七波（最小提交要配多少）见 **§15.0.6**；实现与用法见 `README.md`
